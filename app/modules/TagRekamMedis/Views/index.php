...
<section id="medisPage" class="w-full px-4 py-4 md:py-6 space-y-6 overflow-x-hidden">
    <!-- Header -->
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">
                Kelola tag riwayat medis untuk kategorisasi data pasien
            </p>
        </div>
        ...
        <div class="w-full lg:hidden mt-2">
            <select onchange="window.location.href=this.value" class="w-full bg-indigo-50 border border-indigo-100 text-indigo-700 text-sm font-bold rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-indigo-500/20">
                <option value="<?= site_url('tag-keluhan') ?>">🏷️ Tag Keluhan</option>
    ...
    <div class="rounded-2xl bg-white shadow-sm border border-slate-200/50 overflow-hidden">
        ...
        <div class="border-b border-slate-100 bg-slate-50/50 px-6 py-4">
            ...
            <div class="mb-4">
                <h3 class="text-lg font-semibold text-slate-800">
                    Daftar Tags Riwayat Medis
                </h3>
                <p class="text-sm text-slate-500">
                    Data tag riwayat medis yang terdaftar dalam sistem
                </p>
            </div>
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                    <div class="flex-1 sm:flex-none sm:w-72">
                        <input type="text" id="searchInput" placeholder="Cari tag riwayat medis..."
        ...
        <!-- Mobile Medhis Container (KODE KITA) -->
        <div id="mobile-medhis-container" class="md:hidden divide-y divide-slate-100">
            <div class="px-6 py-12 text-center text-slate-400 italic text-sm">
                <i class="fas fa-spinner fa-spin mr-2 text-slate-300"></i>
                Memuat data tag riwayat medis...
            </div>
        </div>

        <!-- TABLE -->
        <div class="overflow-x-auto hidden md:block">
            <table id="table-medhis" class="w-full text-sm">
                <!-- HEAD -->
        ...
        <div class="border-t border-slate-100 bg-slate-50/30 px-6 py-3 text-xs text-slate-400">
            Data tag riwayat medis yang digunakan untuk kategorisasi
        </div>
    </div>
</section>
...
<div id="modalTambah" class="modal-wrapper hidden fixed inset-0 z-50 items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl">
        <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
            <h5 class="text-lg font-semibold text-slate-800">Tambah Tag Riwayat Medis</h5>
            <button type="button" data-modal-close
                class="rounded-md p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-800">&times;</button>
        </div>

        <form id="addMedhisForm" action="<?= base_url('tag-rekam-medis/store') ?>" method="post" class="space-y-4 p-5">
            <?= csrf_field() ?>

            <div class="space-y-1">
                <label class="text-sm font-medium text-slate-700">Nama Tag Riwayat Medis <span class="text-red-500">*</span></label>
...
<div id="modalEdit" class="modal-wrapper hidden fixed inset-0 z-50 items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl">
        <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
            <h5 class="text-lg font-semibold text-slate-800">Ubah Tag Riwayat Medis</h5>
            <button type="button" data-modal-close
                class="rounded-md p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-800">&times;</button>
        </div>

        <form id="editMedhisForm" action="" method="post" class="space-y-4 p-5">
            <?= csrf_field() ?>
...
<div id="modalDelete" class="modal-wrapper hidden fixed inset-0 z-50 items-center justify-center bg-black/40 p-4">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6 space-y-4">
        ...
        <form id="deleteMedhisForm" action="" method="post" class="flex justify-end gap-2">
            <?= csrf_field() ?>
...
<script>
    window.medisConfig = {
        csrfName: "<?= csrf_token() ?>",
        csrfHash: "<?= csrf_hash() ?>",
        fetchUrl: "<?= site_url('tag-rekam-medis/fetch') ?>",
        storeUrl: "<?= base_url('tag-rekam-medis/store') ?>",
        checkNameUrl: "<?= base_url('tag-rekam-medis/check-name') ?>"
    };
</script>
