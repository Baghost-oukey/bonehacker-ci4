...
<div id="statistikKeuanganPage" class="space-y-6">

    <!-- Header -->
    <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between mb-8">
        <div>
            <h2 class="text-2xl font-extrabold text-slate-800 tracking-tight text-uppercase">Analisis Keuangan</h2>
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <?php if (in_array($role, ['superadmin', 'owner'])): ?>
                <div class="relative">
                    <select id="chartRegionFilter" aria-label="Filter Cabang" class="appearance-none rounded-xl border border-slate-200 bg-white pl-4 pr-10 py-2 text-sm font-bold text-slate-700 shadow-sm focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all outline-none">
            ...
            <!-- DATETIME -->
            <div class="flex bg-slate-100 p-1 rounded-xl border border-slate-200" id="timeFilterGroup">
                <button data-value="7" class="time-filter-btn px-4 py-1.5 text-xs font-bold rounded-lg transition-all active-filter bg-white text-slate-900 shadow-sm">7 Hari</button>
