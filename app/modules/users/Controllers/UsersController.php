...
class UsersController extends BaseController
{
  ...
  public function fetch()
  {
    ...
    foreach ($dataOutput as $value) {
      ...
      $value->action =
        ...
        $value->role .
        '"
                    data-regions_patient="' .
        esc(json_encode($regions_patient_ids)) .
        '"
                    data-href="' .
        base_url('users/update/' . $value->id) .
        '"
                    class="btn btn-primary btn-sm btn_edit"><i class="fas fa-edit"></i></button>
                <button type="button" class="btn btn-info btn-sm btn_add_patient" data-userid="' .
        $value->id .
        '"><i class="fas fa-user-plus"></i></button>
                <button type="button" data-href="' .
        base_url('users/destroy/' . $value->id) .
        '" class="btn btn-danger btn-sm btn_delete"><i class="fas fa-trash"></i></button>';
    }
  ...
  public function view_patient($user_id)
  {
    ...
    return view('App\Modules\Users\Views\UsersLuard\index', $data);
  }
  ...
  public function fetch_patients_luar()
  {
    ...
    $datatables->addColumn('aksi', function ($row) use ($user_id) {
      return '
            <button class="btn btn-danger btn-sm btn-delete-patient mr-1" data-patient-id="' .
        (int) $row->id .
        '" data-user-id="' .
        (int) $user_id .
        '"><i class="fas fa-trash"></i></button>
            <button class="btn btn-success btn-sm btn-send-wa" data-patient-id="' .
        (int) $row->id .
  ...
  public function check_username_exists_edit()
  {
    $username = $this->request->getPost('username');
    $user_id = $this->request->getPost('user_id');
    $exists = $this->model_users->where('username', $username)->where('id !=', $user_id)->countAllResults() > 0;
