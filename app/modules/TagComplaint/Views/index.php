...
<section id="complaintPage" class="w-full px-4 py-4 md:py-6 space-y-6 overflow-x-hidden">
    <!-- Header -->
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">
                Kelola tag keluhan untuk kategorisasi data pasien
            </p>
        </div>
    ...
    <div class="rounded-2xl bg-white shadow-sm border border-slate-200/50 overflow-hidden">
        ...
        <div class="border-b border-slate-100 bg-slate-50/50 px-6 py-4">
            ...
            <div class="mb-4">
                ...
                <p class="text-sm text-slate-500">
                    Data tag keluhan yang terdaftar dalam sistem
                </p>
            </div>
        ...
        <!-- Mobile Complaint Container (KODE KITA) -->
        <div id="mobile-complaint-container" class="md:hidden divide-y divide-slate-100">
            <div class="px-6 py-12 text-center text-slate-400 italic text-sm">
                <i class="fas fa-spinner fa-spin mr-2 text-slate-300"></i>
                Memuat data tag keluhan...
            </div>
        </div>

        <!-- TABLE -->
        <div class="overflow-x-auto hidden md:block">
            <table id="table-complaint" class="w-full text-sm">
                <!-- HEAD -->
        ...
        <div class="border-t border-slate-100 bg-slate-50/30 px-6 py-3 text-xs text-slate-400">
            Data tag keluhan yang digunakan untuk kategorisasi
        </div>
    </div>
</section>
...
<div id="modalTambah" class="modal-wrapper hidden fixed inset-0 z-50 items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl">
        <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
            <h5 class="text-lg font-semibold text-slate-800">Tambah Tag Keluhan</h5>
...
<div id="modalEdit" class="modal-wrapper hidden fixed inset-0 z-50 items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl">
        <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
            <h5 class="text-lg font-semibold text-slate-800">Ubah Tag Keluhan</h5>
            <button type="button" data-modal-close
                class="rounded-md p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-800">&times;</button>
        </div>

        <form id="editComplaintForm" action="" method="post" class="space-y-4 p-5">
            <?= csrf_field() ?>
