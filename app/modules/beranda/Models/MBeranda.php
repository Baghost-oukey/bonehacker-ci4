  ...
  public function getPatientCount($type, $role, $region_patients)
  {
    ...
    switch ($type) {
      ...
      case 'thismonth':
        ...
        break;
      case 'lastmonth':
        $builder->where('MONTH(created_at)', date('m', strtotime('-1 month')))->where('YEAR(created_at)', date('Y', strtotime('-1 month')));
        break;
      case 'thisyear':
  ...
  public function getVisitCount($type, $role, $region_patients)
  {
    ...
    switch ($type) {
      ...
      case 'yesterday':
        ...
        break;
      case 'thismonth':
        $builder->where('MONTH(histories.date)', date('m'))->where('YEAR(histories.date)', date('Y'));
        break;
      case 'lastmonth':
        $lastMonth = date('Y-m-d', strtotime('first day of last month'));
        $builder->where('MONTH(histories.date)', date('m', strtotime($lastMonth)))->where('YEAR(histories.date)', date('Y', strtotime($lastMonth)));
        break;
      case 'thisyear':
