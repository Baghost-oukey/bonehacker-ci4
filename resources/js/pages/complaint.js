...
const setupComplaintPage = () => {
    ...
    const renderEmptyState = (message) => {
        ...
        $("#mobile-complaint-container").html(`<div class="px-6 py-12 text-center text-slate-400 italic text-sm"><i class="fas fa-inbox mr-2 text-slate-300"></i>${message}</div>`);
    };
    const loadTableData = (pageNumber = 1) => {
        ...
        $.ajax({
            ...
            success: (response) => {
                ...
                const mContainer = $("#mobile-complaint-container");
                tbody.empty();
                mContainer.empty();
