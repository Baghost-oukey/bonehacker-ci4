...
const setupResultPage = () => {
    ...
    const renderEmptyState = (message) => {
        ...
        $("#mobile-result-container").html(`<div class="px-6 py-12 text-center text-slate-400 italic text-sm"><i class="fas fa-inbox mr-2 text-slate-300"></i>${message}</div>`);
    };
    ...
    const loadTableData = (pageNumber = 1) => {
        $.ajax({
            ...
            success: (response) => {
                ...
                const mContainer = $("#mobile-result-container");
                tbody.empty();
                mContainer.empty();
